 ...
 * v5.8: Enhanced with full USALI breakdown
 */
function transformPnlToRows(
  pnl: Array<{
    yearIndex: number;
    revenueTotal: number;
...
export function PnLView({ operations, modelOutput }: PnLViewProps) {
  ...
  // Transform to StatementRow format
  const statementRows = useMemo(() => {
    return transformPnlToRows(filteredPnl);
  }, [filteredPnl]);
  ...
  }, [filteredPnl, modelOutput.scenario.startYear]);
  ...
  return (
    ...
    >
      <div>
        ...
        >
          Consolidated P&L for selected operations
        </p>
      </div>
          ...
          {statementRows.length > 0 ? (
            ...
            >
              <p style={{ margin: 0, fontSize: '0.9375rem' }}>
                Select at least one operation to view P&L statement
              </p>
            </div>
          )}
        </div>

...
export function OperationsView({
  ...
  const handleAddAsset = (operationType?: string) => {
    // Placeholder for adding new asset
    // This would typically open a modal or navigate to an add form
    console.log('Add asset:', operationType);
  };

  const handleReorder = (reorderedOperations: OperationConfig[]) => {
    if (onOperationsChange) {
      onOperationsChange(reorderedOperations);
    }
  };
  ...
  const masterPanel = (
    <OperationList
      ...
      onAddAsset={onOperationsChange ? handleAddAsset : undefined}
      onReorder={onOperationsChange ? handleReorder : undefined}
    />
  );
    ...
    >
      <div style={{ textAlign: 'center' }}>
        ...
        <p style={{ fontSize: '0.875rem' }}>
          Select an asset from the list to view and edit its configuration
        </p>
      </div>
    </div>
  );

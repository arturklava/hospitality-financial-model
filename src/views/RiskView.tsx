  ...
  const handleApplyGoalSeekResult = (optimizedValue: number, inputVariable: string) => {
    ...
    switch (inputVariable) {
      ...
      case 'initialInvestment':
        ...
        break;
      case 'debtAmount': {
        if (updatedInput.capitalConfig.debtTranches.length > 0) {
          const tranches = updatedInput.capitalConfig.debtTranches.map((tranche, index) => {
            if (index === 0) {
              return { ...tranche, initialPrincipal: optimizedValue };
            }
            return tranche;
          });
          updatedInput.capitalConfig = { ...updatedInput.capitalConfig, debtTranches: tranches };
        }
        break;
      }
      case 'interestRate': {
  ...
  return (
    <div className="risk-view">
      <div className="view-header" style={{ marginBottom: '2rem' }}>
        <h1 style={{ fontFamily: 'var(--font-display, "Josefin Sans", sans-serif)' }}>Risk Intelligence</h1>
        <p style={{ color: 'var(--text-secondary)', fontFamily: 'var(--font-body, "Montserrat", sans-serif)' }}>
          Probabilistic risk analysis through Monte Carlo simulation and deterministic sensitivity analysis.
        </p>
      </div>
      ...
      <div style={{ 
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          <label style={{ fontWeight: 500, fontFamily: 'var(--font-body, "Montserrat", sans-serif)' }}>Analyze KPI:</label>
          <select
            value={selectedKpi}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          <label style={{ fontWeight: 500, fontFamily: 'var(--font-body, "Montserrat", sans-serif)' }}>Iterations:</label>
          <input
            type="number"
            min="100"
            max="10000"
          ...
          <div style={{ marginTop: '1.5rem', paddingTop: '1.5rem', borderTop: '1px solid var(--border-color, #e0e0e0)' }}>
            ...
            {!correlationMatrix && (
              <p style={{ 
                ...
                Correlation matrix is disabled. Variables will be sampled independently.
                Click "Enable Correlation Matrix" to configure correlations between variables.
              </p>
            )}
          </div>
        </SectionCard>
      ...
      {!isLoading && (
        <div style={{ marginTop: '2rem', marginBottom: '2rem' }}>
          <GoalSeekPanel
            ...
            currentModelOutput={baseOutput}
            onApplyResult={handleApplyGoalSeekResult}
          />
        </div>
      )}
      ...
      {!isLoading && (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2rem', marginTop: '2rem' }}>
          ...
          <div className="card">
            <h2 style={{ marginTop: 0, marginBottom: '1rem' }}>Monte Carlo Distribution</h2>
            {simulationResult && kpiValues.length > 0 ? (
              ...
              <div style={{ 
                ...
                <p>Run a simulation to view the distribution chart.</p>
              </div>
            )}
          </div>
          ...
          <div>
            <SensitivityPanel baseScenario={baseScenario} />
          </div>
        </div>
      )}
    </div>

...
export function LandView({ input, onProjectConfigChange }: LandViewProps) {
  ...
  const paymentSchedule = useMemo(() => {
    ...
    if (selectedLand.downPayment > 0) {
      schedule.push({
        ...
        monthLabel: `Month ${selectedLand.downPaymentMonth}`,
    ...
    if (selectedLand.installments && selectedLand.installments.length > 0) {
      selectedLand.installments.forEach((installment) => {
        schedule.push({
          ...
          monthLabel: `Month ${installment.month}`,
    ...
    if (selectedLand.barterValue && selectedLand.barterValue > 0 && selectedLand.barterMonth !== undefined) {
      ...
      schedule.push({
        ...
        monthLabel: `Month ${selectedLand.barterMonth}`,
        amount: barterAmount,
        type: 'Barter',
      });
    }
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
        ...
        {landConfigs.length === 0 && (
          <div style={{ padding: '2rem', textAlign: 'center', color: 'var(--text-secondary)', fontFamily: 'var(--font-body, "Montserrat", sans-serif)' }}>
            No land acquisitions configured. Click "Add" to create one.
          </div>
        )}
      </div>
    </div>
  ...
  const detailPanel = selectedLand ? (
    <div style={{ padding: '2rem', display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
      ...
      <div>
        ...
        <p style={{ color: 'var(--text-secondary)', margin: 0, fontFamily: 'var(--font-body, "Montserrat", sans-serif)' }}>
          Configure land acquisition details and payment schedule.
        </p>
      </div>
      ...
      <SectionCard title="Acquisition Details">
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '1.5rem' }}>
          <InputGroup label="Land Name" required>
            <input
              type="text"
          ...
          <InputGroup label="Total Acquisition Cost" required>
            <CurrencyInput
          ...
          <InputGroup label="Acquisition Month" helperText="Negative = before Year 0">
            <input
              type="number"
      ...
      <SectionCard title="Payment Structure">
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '1.5rem' }}>
          <InputGroup label="Down Payment" required>
            <CurrencyInput
          ...
          <InputGroup label="Down Payment Month">
            <input
              type="number"
          ...
          <InputGroup 
            label="Barter %"
            helperText="Percentage of total cost (0-1)"
          >
            <input
              type="number"
              min="0"
          ...
          <div style={{ width: '100%', height: '400px' }}>
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={paymentSchedule} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                ...
                />
                <Legend />
                <Bar dataKey="amount" fill="var(--primary)" name="Payment Amount" />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </SectionCard>
      ...
      {paymentSchedule.length === 0 && (
        <SectionCard title="Payment Schedule">
          <div style={{ padding: '2rem', textAlign: 'center', color: 'var(--text-secondary)', fontFamily: 'var(--font-body, "Montserrat", sans-serif)' }}>
            Configure down payment and installments to see payment schedule.
          </div>
        </SectionCard>
      )}
    </div>
    ...
    <div style={{ padding: '2rem', textAlign: 'center', color: 'var(--text-secondary)', fontFamily: 'var(--font-body, "Montserrat", sans-serif)' }}>
      {landConfigs.length === 0
        ? 'No land acquisitions configured. Click "Add" in the sidebar to create one.'
        : 'Select a land acquisition from the sidebar to view details.'}
    </div>
  );

...
function buildAssumptions(wb: ExcelJS.Workbook, scenario: NamedScenario, output: FullModelOutput): GlobalMap {
  ...
  const debtTranches = scenario.modelConfig.capitalConfig.debt;
  if (debtTranches.length === 0) {
    ws.getCell(`B${r}`).value = "No Debt used in this scenario";
    applyStyle(ws.getCell(`B${r}`), 'label');
    r++;
  } else {
    const debtHeaders = ["Tranche", "Principal", "Rate", "Term (Y)"];
    ...
    debtTranches.forEach((tranche) => {
      ws.getCell(r, 2).value = tranche.label || tranche.id;
      applyStyle(ws.getCell(r, 2), 'label');

      ws.getCell(r, 3).value = tranche.initialPrincipal || tranche.amount || 0;
      ws.getCell(r, 3).numFmt = STYLES.formats.curr;
      applyStyle(ws.getCell(r, 3), 'input');

      ws.getCell(r, 4).value = tranche.interestRate;
      ws.getCell(r, 4).numFmt = STYLES.formats.pct;
      applyStyle(ws.getCell(r, 4), 'input');

      ws.getCell(r, 5).value = tranche.termYears;
      ws.getCell(r, 5).numFmt = STYLES.formats.int;
      applyStyle(ws.getCell(r, 5), 'input');
 ...
 * 6. DEBT SCHEDULE
 */
function buildDebt(wb: ExcelJS.Workbook, output: FullModelOutput, horizon: number, startYear: number) {
  ...
  const tranches = output.capital.trancheSchedules || [];

  let r = 4;
  const headers = ['Year', 'Beg. Balance', 'Interest', 'Principal', 'Total Service', 'End. Balance'];

  // Add headers for individual tranches if multiple
  if (tranches.length > 1) {
    tranches.forEach(t => {
      headers.push(`${t.trancheId} Service`);
    });
  }
  ...
  const schedule = output.capital.debtSchedule.entries;

  for (let i = 0; i < horizon; i++) {
    const row = ws.getRow(r);
    const entry = schedule[i] || { beginningBalance: 0, interest: 0, principal: 0, endingBalance: 0 };

    row.getCell(2).value = `Year ${startYear + i}`;
    row.getCell(3).value = entry.beginningBalance;
    row.getCell(4).value = entry.interest;
    row.getCell(5).value = entry.principal;
    row.getCell(6).value = entry.interest + entry.principal;
    row.getCell(7).value = entry.endingBalance;

    for (let c = 3; c <= 7; c++) {
      applyStyle(row.getCell(c), 'formula'); // Using 'formula' style for values even if exported as static
      row.getCell(c).numFmt = STYLES.formats.num;
    }

    // Individual Tranche Service columns
    if (tranches.length > 1) {
      tranches.forEach((t, tIdx) => {
        const tEntry = t.entries[i];
        const tService = tEntry ? (tEntry.interest + tEntry.principal) : 0;
        const col = 8 + tIdx;
  ...
  if (tranches.length > 1) {
    tranches.forEach((_, i) => {
      ws.getColumn(8 + i).width = 18;
    });
  }
}
...
function buildPnL(
  ...
  if (mode === 'consol') {
    ...
    if (debtTranches.length <= 1) {
      addLine("Debt Service", 'debtService', 'formula', (_c, i) => {
        return `'Debt'!F${5 + i}`;
      });
    } else {
      // Multiple tranches: show breakdown
      debtTranches.forEach((tranche, tIdx) => {
        const label = `Debt Service - ${tranche.label || tranche.id}`;
        const colLetter = getCol(8 + tIdx);
        addLine(label, 'debtService', 'formula', (_c, i) => {
          return `'Debt'!${colLetter}${5 + i}`;
        }, 1, 1);
      });
      // Grand total
      addLine("Total Debt Service", 'debtService', 'total', (_c, i) => {
        return `'Debt'!F${5 + i}`;
      });
    }

    addLine("LEVERED FREE CASH FLOW", 'cashFlow', 'total', (c) => `${c}${ufcfRow}+${c}${r - 1}`);
  } else {
...
export async function generateExcel(scenario: NamedScenario, output: FullModelOutput): Promise<void> {
  ...
  buildDebt(wb, output, scenario.modelConfig.scenario.horizonYears, scenario.modelConfig.scenario.startYear);
  ...
  scenario.modelConfig.scenario.operations.forEach(op => {
    ...
    if (data.length > 0) {
      ...
      opNames.push(sName);
      buildPnL(wb, sName, data, scenario.modelConfig.scenario.horizonYears, scenario.modelConfig.scenario.startYear, 'op', map, scenario, op.id);
    }
  });
  ...
  buildPnL(wb, 'Consolidated', consolData, scenario.modelConfig.scenario.horizonYears, scenario.modelConfig.scenario.startYear, 'consol', map, scenario, undefined, opNames);

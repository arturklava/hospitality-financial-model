...
export function runProjectEngine(
  ...
  // vPatch1: Invariante Check
  try {
    validateProjectInvariants(data);
  } catch (e) {
    return engineFailure(
      'PROJECT_INVARIANCE_FAILURE',
      e instanceof Error ? e.message : 'Financial invariants failed',
      {
        auditTrace,
        issues: [{ path: 'projectInvariants', message: e instanceof Error ? e.message : 'Unknown invariance error', code: 'custom' }]
      }
    );
  }
...
/**
 * Validates financial invariants to ensure accounting truth.
 * Throws error if impossible state is detected.
 */
function validateProjectInvariants(result: ProjectEngineRunResult): void {
  // 1. NaN checks
  if (Number.isNaN(result.projectKpis.npv)) {
    throw new Error('Invariant Failed: NPV is NaN');
  }
  if (result.projectKpis.unleveredIrr !== null && Number.isNaN(result.projectKpis.unleveredIrr)) {
    throw new Error('Invariant Failed: IRR is NaN (should be null or number)');
  }

  // 3. Infinite Check
  if (!Number.isFinite(result.projectKpis.npv) && result.projectKpis.npv !== 0) {
    // We allow 0, but infinite NPV usually signals an error in discount vs growth formulas
    // For now, we flag it.
    // throw new Error('Invariant Failed: NPV is Infinite');
  }
}

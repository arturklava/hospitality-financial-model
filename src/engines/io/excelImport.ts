...
function buildFullModelInputFromKeyValueMap(keyValueMap: Map<string, string | number>): FullModelInput {
  ...
  const debtAmount = getNumberOptional(keyValueMap, 'capitalStructure.debtTranches[0].initialPrincipal') ||
                     getNumberOptional(keyValueMap, 'capitalStructure.debtTranches[0].amount');
  if (debtAmount !== undefined && capitalConfig.debtTranches.length > 0) {
    capitalConfig.debtTranches[0] = {
      ...capitalConfig.debtTranches[0],
      initialPrincipal: debtAmount,
      amount: debtAmount, // backward compatibility
      interestRate: getNumber(keyValueMap, 'capitalStructure.debtTranches[0].interestRate', 0.08),
      termYears: getNumber(keyValueMap, 'capitalStructure.debtTranches[0].termYears', 10),
      amortizationYears: getNumber(keyValueMap, 'capitalStructure.debtTranches[0].amortizationYears', 10),
    };
  }

...
function applyVariableAdjustment(
  ...
  switch (variable) {
    ...
    case 'debtAmount': {
      // Multiplicative adjustment to debt amount (all tranches proportionally)
      if (capitalConfig.debt.length > 0) {
        capitalConfig.debt.forEach(tranche => {
    ...
    case 'interestRate': {
      // Multiplicative adjustment to interest rate (all tranches)
      capitalConfig.debt.forEach(tranche => {
...
function getBaseValue(variable: SensitivityVariable, baseScenario: NamedScenario): number {
  ...
  switch (variable) {
    ...
    case 'debtAmount':
      if (capitalConfig.debt.length > 0) {
        const firstTranche = capitalConfig.debt[0];
    ...
    case 'interestRate':
      if (capitalConfig.debt.length > 0) {
        return capitalConfig.debt[0].interestRate;
      }
      return 0;

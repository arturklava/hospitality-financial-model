 ...
 */
function validateTrancheConfig(tranche: DebtExecution): void {
  ...
  if (initialPrincipal > 0 && tranche.termYears < 0) {
    throw new Error(`[Capital Engine] Invalid tranche ${tranche.id}: termYears must be >= 0.`);
  }
...
function calculateTransactionCosts(
  ...
  for (let yearIndex = 0; yearIndex < horizonYears; yearIndex++) {
    const maturityYear = startYear + term - 1;
    const isEarlyRefinance = refinanceAtYear !== undefined && refinanceAtYear < maturityYear;

    const isMaturityYear = yearIndex === maturityYear && !isEarlyRefinance;
    const isRefinancingYear = refinanceAtYear !== undefined && yearIndex === refinanceAtYear;
...
export function runCapitalEngine(
  ...
  // v0.12: Unlevered Scenario Toggle
  // If isUnleveredScenario is true, we force a zero-debt outcome
  if (capitalConfig.isUnleveredScenario) {
    const entries: DebtScheduleEntry[] = [];
    const monthlyEntries: MonthlyDebtScheduleEntry[] = [];
    const leveredFcfByYear: LeveredFcf[] = [];
    const ownerLeveredCashFlows: number[] = [];
    const debtKpis: DebtKpi[] = [];
    const monthlyDebtKpis: MonthlyDebtKpi[] = [];

    // Create zero entries for the entire horizon
    for (let yearIndex = 0; yearIndex < horizonYears; yearIndex++) {
      // Debt Schedule
      entries.push({
        yearIndex,
      ...
      // Levered FCF = Unlevered FCF (since no debt service)
      const uFcf = unleveredFcf[yearIndex];
      leveredFcfByYear.push({
        yearIndex,
        unleveredFcf: uFcf.unleveredFreeCashFlow,
        debtService: 0,
        interest: 0,
        principal: 0,
        transactionCosts: 0,
        leveredFreeCashFlow: uFcf.unleveredFreeCashFlow,
      });

      // Owner Cash Flows (Year 1..N)
      ownerLeveredCashFlows.push(uFcf.unleveredFreeCashFlow);

      // Debt KPIs (No debt => null DSCR/LTV or handle as needed)
      debtKpis.push({
        yearIndex,
        dscr: null,
        ltv: 0,
    ...
    if (consolidatedMonthlyPnl) {
      ...
      for (let i = 0; i < totalMonths; i++) {
        ...
        monthlyDebtKpis.push({
          ...
          monthIndex,
          monthNumber: i,
          dscr: null, // No debt service
          ltv: 0,
        });

        // Monthly Cash Flow
        const mPnl = consolidatedMonthlyPnl.find(p => p.monthNumber === i);
        if (mPnl) {
          const val = mPnl.noi - mPnl.maintenanceCapex; // - 0 debt
          cumulativeCashFlow += val;
          mCashFlow.push({
            yearIndex,
            monthIndex,
            monthNumber: i,
            noi: mPnl.noi,
            debtService: 0,
            maintenanceCapex: mPnl.maintenanceCapex,
            monthlyCashFlow: val,
            cumulativeCashFlow,
    ...
    // Prepend initial investment (negative) to owner cash flows for Year 0
    // But wait, ownerLeveredCashFlows usually starts at Year 1 in some contexts, 
    // but in Waterfall it usually expects Year 0. 
    // Looking at the return type `ownerLeveredCashFlows: number[] // length N+1: Year0..YearN`
    // So we need to add Year 0.
    const ownerCashFlowsWithYear0 = [-initialInvestment, ...ownerLeveredCashFlows];

    return {
      debtSchedule: { entries },
      trancheSchedules: [], // No debt => empty tranche schedules
      monthlyDebtSchedule,
      leveredFcfByYear,
      monthlyCashFlow,
      ownerLeveredCashFlows: ownerCashFlowsWithYear0,
      debtKpis,
      monthlyDebtKpis: monthlyDebtKpis.length > 0 ? monthlyDebtKpis : undefined,
      covenantStatus: [], // No covenants in unlevered
    };
  }
  ...
  return {
    debtSchedule,
    trancheSchedules, // v5.9: Exposed for visualization
    monthlyDebtSchedule,
    leveredFcfByYear,
    monthlyCashFlow,

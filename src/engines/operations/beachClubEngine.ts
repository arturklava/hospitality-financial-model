/**
 * Beach Club operation engine.
 * This will generate monthly and annual P&L statements for beach club operations.
...
import { aggregateAnnualPnl, applyRampUp, applySeasonality, DAYS_PER_MONTH, getSeasonalityCurve, MONTHS_PER_YEAR } from './utils';

export interface BeachClubEngineResult {
  monthlyPnl: MonthlyPnl[];
  annualPnl: AnnualPnl[];
}

/**
 * Runs the beach club operation engine to generate monthly and annual P&L statements.
 *
 * @param config - Beach club configuration
 * @returns Monthly and annual P&L results
 */
export function runBeachClubEngine(config: BeachClubConfig): BeachClubEngineResult {
  ...
  const monthlyMembershipRevenue = (config.memberships * config.avgMembershipFee) / MONTHS_PER_YEAR;
  ...
  for (let yearIndex = 0; yearIndex < config.horizonYears; yearIndex++) {
    ...
    for (let monthIndex = 0; monthIndex < 12; monthIndex++) {
      ...
      // Total revenue = primaryRevenue / (1 - foodRevenuePctOfTotal - beverageRevenuePctOfTotal - otherRevenuePctOfTotal)
      // But we calculate it differently: primary revenue is the base, and food/beverage/other are percentages of total
      // So: totalRevenue = primaryRevenue + foodRevenue + beverageRevenue + otherRevenue
      // Where: foodRevenue = totalRevenue × foodRevenuePctOfTotal
      // Solving: totalRevenue = primaryRevenue / (1 - foodRevenuePctOfTotal - beverageRevenuePctOfTotal - otherRevenuePctOfTotal)

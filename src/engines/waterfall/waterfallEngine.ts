...
export function applyEquityWaterfall(
  ...
  if (useMultiTier) {
    result = applyMultiTierWaterfall(ownerCashFlows, config, classes);
  } else {
    result = applySingleTierWaterfall(ownerCashFlows, config, classes);
  }

  // vPatch1: Invariante Check (Conservation of Cash)
  validateWaterfallInvariants(result);
  return result;
}
...
/**
 * Validates waterfall invariants:
 * 1. Conservation of Cash: Sum(Partner Flows) == Owner Flow (at every t)
 */
function validateWaterfallInvariants(result: WaterfallResult): void {
  const n = result.ownerCashFlows.length;
  // Tolerance for floating point (e.g. billion dollar project, cent precision)
  const EPSILON = 1e-5;

  for (let t = 0; t < n; t++) {
    const ownerCf = result.ownerCashFlows[t];
    let partnerSum = 0;
    for (const partner of result.partners) {
      partnerSum += partner.cashFlows[t] ?? 0;
    }

    if (Math.abs(partnerSum - ownerCf) > EPSILON) {
      throw new Error(
        `Waterfall Invariant Failed: Cash conservation at year ${t}. ` +
        `Owner: ${ownerCf.toFixed(4)}, Partners Sum: ${partnerSum.toFixed(4)}, Diff: ${(partnerSum - ownerCf).toFixed(6)}`
      );
    }
  }
}

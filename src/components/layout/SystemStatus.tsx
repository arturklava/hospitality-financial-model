...
import { useModelStatus } from '../../ui/contexts/ModelStatusContext';
...
export function SystemStatus() {
  ...
  const getStatusText = (): string => {
    // Priority: Live Async Status
    if (modelStatus === 'computing') return 'Calculando...';
    if (modelStatus === 'syncing') return 'Sincronizando...';
  ...
  // Determine status color (Moved down after modelStatus check)
  const getStatusColor = (): string => {
    if (modelStatus === 'computing') return '#eab308'; // Yellow
    if (modelStatus === 'syncing') return '#f97316'; // Orange

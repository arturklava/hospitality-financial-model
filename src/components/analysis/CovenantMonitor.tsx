/**
 * Covenant Monitor Component (v2.2: Liquidity Dashboard)
 * 
 * Displays covenant compliance status with color-coded cards.
 * Shows Green (Safe), Yellow (Near Breach/Warning), Red (Breach/Critical).
 * Click on a covenant to view breach timeline.
 */
...
function formatMonthLabel(yearIndex: number, monthIndex: number): string {
  ...
  return `Year ${yearIndex + 1}, ${months[monthIndex]}`;
}
 ...
 * Get status label.
 */
function getStatusLabel(status: 'safe' | 'warning' | 'breach'): string {
  switch (status) {
    case 'safe':
      return 'Safe';
    case 'warning':
...
function CovenantCard({ covenant, summary, onViewTimeline }: CovenantCardProps) {
  ...
  const statusLabel = getStatusLabel(summary.overallStatus);
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div style={{ flex: 1 }}>
          ...
          <p style={{ margin: 0, fontSize: '0.875rem', color: 'var(--text-secondary, #666)' }}>
            Type: {covenant.type === 'min_dscr' ? 'Minimum DSCR' : covenant.type === 'max_ltv' ? 'Maximum LTV' : 'Minimum Cash'}
          </p>
        ...
        <div style={{
          ...
          <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary, #666)' }}>
            {summary.breached > 0 ? `${summary.breached} breaches` : 'No breaches'}
          </div>
        </div>
      </div>
        ...
        <div style={{ 
          ...
          <div style={{ display: 'flex', gap: '1rem' }}>
            <span style={{ color: 'var(--text-secondary, #666)' }}>
              Passed: <strong>{summary.passed}</strong> months
            </span>
            <span style={{ color: '#f44336' }}>
              Breached: <strong>{summary.breached}</strong> months
            </span>
            {summary.critical > 0 && (
    ...
    >
      ...
      >
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
          <h2 style={{ margin: 0 }}>Breach Timeline: {covenant.name}</h2>
          <button
            onClick={onClose}
            style={{
              padding: '0.5rem',
...
export function CovenantMonitor({ covenantStatus = [], covenants = [] }: CovenantMonitorProps) {
  ...
  if (covenants.length === 0) {
    return (
      <div className="card">
        <h3 style={{ marginTop: 0 }}>Covenant Monitor</h3>
        <p style={{ color: 'var(--text-secondary, #666)', fontStyle: 'italic' }}>
          No covenants defined. Configure covenants in Capital Structure settings.
        </p>
      </div>
    );
  }
  ...
  return (
    <>
      <div>
        <h3 style={{ marginBottom: '1rem' }}>Covenant Monitor</h3>

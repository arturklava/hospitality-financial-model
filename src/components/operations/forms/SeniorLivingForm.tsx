...
interface SeniorLivingFormProps {
  ...
  onChange?: (updates: Partial<SeniorLivingConfig>) => void;
  readOnly?: boolean;
}

export function SeniorLivingForm({ operation, onChange, readOnly = false }: SeniorLivingFormProps) {
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          ...
          <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
              ...
              <InputGroup
                ...
                required
                helperText="Typical range: $2,000 - $15,000 per month"
                error={
                  operation.avgMonthlyRate <= 0
                    ? 'Monthly rate must be greater than $0'
                    : operation.avgMonthlyRate > 50000
                      ? 'Monthly rate seems unusually high. Please verify the value.'
                      : undefined
                }
              >
                <CurrencyInput
                  value={operation.avgMonthlyRate}
                  onChange={(value) => {
                    const validatedValue = Math.max(0, value);
                    onChange({ avgMonthlyRate: validatedValue });
                  }}
                  style={{
                    width: '100%',
                    padding: '0.75rem',
          ...
          <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
              <PercentageSlider
                label="Care Revenue %"
                value={operation.careRevenuePctOfRental}
                onChange={(value) => onChange({ careRevenuePctOfRental: clampPct(value) })}
                min={0}
                max={1}
                step={0.01}
              />
              ...
              <PercentageSlider
                ...
                onChange={(value) => onChange({ foodRevenuePctOfRental: clampPct(value) })}
                min={0}
                max={1}
                step={0.01}
              />
              ...
              <PercentageSlider
                ...
                onChange={(value) => onChange({ otherRevenuePctOfRental: clampPct(value) })}
                min={0}
                max={1}
                step={0.01}
              />
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          ...
          <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
              ...
              <PercentageSlider
                label="Care COGS %"
                value={operation.careCogsPct}
                onChange={(value) => onChange({ careCogsPct: clampPct(value) })}
                min={0}
                max={1}
                step={0.01}
              />

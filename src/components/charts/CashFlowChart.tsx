...
export function CashFlowChart({ consolidatedPnl, leveredFcf }: CashFlowChartProps) {
  ...
  const chartData = (consolidatedPnl || []).map((pnl) => {
    ...
    return {
      ...
      gop: pnl.gop ?? (pnl.revenueTotal - (pnl.departmentalExpenses ?? pnl.cogsTotal)),
      noi: pnl.noi,
      debtService: -(levered?.debtService ?? 0),
      replacementReserve: -(pnl.maintenanceCapex ?? 0),
      leveredFcf: levered?.leveredFreeCashFlow ?? 0,
    };
  });

  // Validate data: Must have data and at least one non-zero/non-NaN value to show something
  const hasValidData = chartData.length > 0 && chartData.some(d =>
    !Number.isNaN(d.noi) && !Number.isNaN(d.leveredFcf)
  );

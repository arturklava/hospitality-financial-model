...
export function CapitalStackChart({ capitalConfig }: CapitalStackChartProps) {
  ...
  const totalInvestment = normalized.initialInvestment;
  const seniorDebt = normalized.seniorDebt;
  const mezzDebt = normalized.mezzDebt;
  const otherDebt = normalized.otherDebt;
  ...
  const hasValidData = !(totalInvestment === 0 && totalCapital === 0) && chartData && chartData.length > 0;

...
describe('Capital Engine', () => {
  ...
  describe('Stack Misto (Senior + Mezz)', () => {
    it('should show different Levered IRR when a mezzanine tranche is added', () => {
      const consolidatedPnl = createConsolidatedPnl(5, 50000000); // 50M NOI
      const unleveredFcf = createUnleveredFcf(5, 40000000);      // 40M UFCF

      const seniorOnlyConfig: CapitalStructureConfig = {
        initialInvestment: 100000000,
        debt: [
          {
            id: 'senior',
            initialPrincipal: 60000000,
            interestRate: 0.05,
            termYears: 10,
      ...
      const resultSenior = runCapitalEngine(consolidatedPnl, unleveredFcf, seniorOnlyConfig);
      const irrSenior = irr(resultSenior.ownerLeveredCashFlows) || 0;

      const mixedConfig: CapitalStructureConfig = {
        initialInvestment: 100000000,
        debt: [
          {
            id: 'senior',
      ...
      const resultMixed = runCapitalEngine(consolidatedPnl, unleveredFcf, mixedConfig);
      const irrMixed = irr(resultMixed.ownerLeveredCashFlows) || 0;

      // Levered IRR should be different (likely higher in this case due to more leverage, 
      // even if more expensive, if projects yield > cost)
      expect(irrMixed).not.toBe(irrSenior);

      // Equity check: Senior only equity = 40M. Mixed equity = 30M.
      expect(resultSenior.ownerLeveredCashFlows[0]).toBe(-40000000);
      expect(resultMixed.ownerLeveredCashFlows[0]).toBe(-30000000);
    });
  });
  ...
  describe('Debt schedule invariant', () => {
    it('should satisfy debt invariant for no-debt scenario', () => {
      ...
      const totalPrincipalPaid = result.debtSchedule.entries.reduce((sum: number, entry: any) => sum + entry.principal, 0);
    ...
    it('should satisfy debt invariant for standard amortization scenario', () => {
      ...
      const totalPrincipalPaid = entries.reduce((sum: number, entry: any) => sum + entry.principal, 0);
    ...
    it('should match v0.4 expectations with single-tranche config', () => {
      ...
      const capitalConfig = buildSingleTrancheCapitalConfig({
        ...
        debt: [
          {
            id: 'senior-loan',
            initialPrincipal: 60000, // v0.4 style
            interestRate: 0.10,
            termYears: 5,
            amortizationYears: 5,
          },
  ...
  describe('Multi-Tranche Aggregation (CRITICAL)', () => {
    it('should correctly sum Senior and Mezzanine tranches', () => {
      const consolidatedPnl = createConsolidatedPnl(5, 5000000); // Plenty of NOI
      const unleveredFcf = createUnleveredFcf(5, 5000000);
      const capitalConfig: CapitalStructureConfig = {
      ...
      // Principal = 10M / 5 = 2,000,000 (approx for linear, check engine logic)
      // Mortgage amortization formula logic:
      // monthlyRate = 0.05/12
      // n = 60
      // payment = P * ...
      // For simplicity in this test context, we just want to ensure SUM is correct.

      const schedule = result.debtSchedule.entries;

      // We can't easily assert exact Mortgage principal without re-implementing the math,
      // but we CAN assert that Total Debt Service = T1 Service + T2 Service.

      // We'll run a single-tranche run for Senior to get its exact values
      const seniorConfig = { ...capitalConfig, debt: [capitalConfig.debt[0]] };
      const seniorResult = runCapitalEngine(consolidatedPnl, unleveredFcf, seniorConfig);
      const seniorServiceY0 = seniorResult.debtSchedule.entries[0].interest + seniorResult.debtSchedule.entries[0].principal;

      // We'll run a single-tranche run for Mezz to get its exact values
      const mezzConfig = { ...capitalConfig, debt: [capitalConfig.debt[1]] };
      const mezzResult = runCapitalEngine(consolidatedPnl, unleveredFcf, mezzConfig);
      const mezzServiceY0 = mezzResult.debtSchedule.entries[0].interest + mezzResult.debtSchedule.entries[0].principal;

      // Expected Total
      const expectedTotalServiceY0 = seniorServiceY0 + mezzServiceY0;
      const actualTotalServiceY0 = schedule[0].interest + schedule[0].principal;

      // The Engine DOES aggregate, so this should match exactly.
      expect(actualTotalServiceY0).toBeCloseTo(expectedTotalServiceY0, 2);

      // Also check Interest Aggregation
      const totalInterestY0 = schedule[0].interest;
      const expectedInterestY0 = (10000000 * 0.05) + (2000000 * 0.10); // 500k + 200k = 700k
      expect(totalInterestY0).toBeCloseTo(expectedInterestY0, 2);

      // Also check Principal Aggregation
      const totalPrincipalY0 = schedule[0].principal;
      // Mezz is IO, so principal is 0. Senior is Mortgage.
      const seniorPrincipalY0 = seniorResult.debtSchedule.entries[0].principal;
      expect(totalPrincipalY0).toBeCloseTo(seniorPrincipalY0, 2);
    });
  });
    ...
    it('should handle backward compatibility with v0.4 amount field', () => {
      ...
      const capitalConfig: CapitalStructureConfig = {
        ...
        debt: [
          {
            id: 'v0.4-tranche',
            initialPrincipal: 60000, // v0.4 field
            interestRate: 0.10,
            termYears: 5,
            amortizationYears: 5,
          },
  ...
  describe('v0.5: Simple refinancing', () => {
    it('should fully repay tranche at refinanceAtYear and start new tranche', () => {
      ...
      const originalLoanPrincipalPaid = entries.slice(0, 3).reduce((sum: number, entry: any) => sum + entry.principal, 0);
  ...
  describe('Unlevered Toggle (Patch 3)', () => {
    it('should force zero debt service when isUnleveredScenario is true', () => {
      const consolidatedPnl = createConsolidatedPnl(5, 100000);
      const unleveredFcf = createUnleveredFcf(5, 80000);
      const capitalConfig: CapitalStructureConfig = {
        initialInvestment: 500000,
        isUnleveredScenario: true, // TOGGLE ON
        debt: [
          {
            id: 'huge-debt',
            initialPrincipal: 500000000, // Massive debt that would crash the model
            interestRate: 0.20,
            termYears: 5,
            amortizationYears: 5,
          },
      ...
      // Verify Levered FCF equals Unlevered FCF
      for (let i = 0; i < 5; i++) {
        expect(result.leveredFcfByYear[i].leveredFreeCashFlow).toBe(result.leveredFcfByYear[i].unleveredFcf);
      }

      // Verify Owner Cash Flows (first is -InitialInvestment, others are FCF)
      expect(result.ownerLeveredCashFlows[0]).toBe(-500000);
      expect(result.ownerLeveredCashFlows[1]).toBe(80000);

      // Verify DSCR is null (not NaN or Infinity)
      expect(result.debtKpis[0].dscr).toBeNull();
    });
  });
});

 ...
 */
export interface DebtExecution {
  id: string;
...
/**
 * Legacy alias for DebtExecution.
 * @deprecated Use DebtExecution instead.
 */
export type DebtTrancheConfig = DebtExecution;
 ...
 * @property tranches - Array of debt tranche configurations (v0.5 supports multiple tranches). Replaces legacy `debtTranches`.
 *
 * @note Backwards compatibility (v0.4 → v0.5):
 * - v0.4 single-tranche configs can be migrated by wrapping the old `DebtExecution` (with `amount` field) into a 1-element array
...
export interface CapitalStructureConfig {
  ...
  debt: DebtExecution[]; // NEW: supports multiple tranches
  tranches?: DebtExecution[];      // DEPRECATED: legacy alias
  covenants?: Covenant[];  // v2.2: Optional covenants for monitoring
  isUnleveredScenario?: boolean; // v0.12: Optional flag to simulate unlevered scenario (all equity) without removing debt inputs
}
...
// Capital engine result (moved definition down to update it)
...
export interface WaterfallTier {
  ...
  catchUpPct?: number;                           // optional, catch-up percentage (0..1)
...
// v5.9: Granular Debt Schedules
export interface TrancheSchedule {
  trancheId: string;
  entries: DebtScheduleEntry[];
  originationFee: number;
  exitFeeByYear: number[];
}

// Capital engine result
export interface CapitalEngineResult {
  debtSchedule: DebtSchedule;
  trancheSchedules: TrancheSchedule[]; // v5.9: Breakdown by tranche
